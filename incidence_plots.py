...
model_labels = {"EBMA": "DSMA", "Static": "CM"}
...
for j, R0 in enumerate(R0_values):
    ...
    for model in ["EBMA", "Static"]:
        ...
        label_prefix = model_labels[model]

        h1, = axI.plot(d["time"], d["I"], color=colors[model],
                       linestyle="-", label=f"{label_prefix} $I(t)$")
        h2, = axk.plot(d["time"], d["mean_kI"], color=colors[model],
                       linestyle=":", label=f"{label_prefix} ⟨k⟩₍ᵢ₎")
    ...
    for model in ["EBMA", "Static"]:
        ...
        hR, = axR.plot(d["time"], d["R"], color=colors[model],
                       linestyle="-", label=f"{label_prefix} $R(t)$")
        if j == 0:
...
for ax in axes[1, :]:
    ax.set_xlabel(r"Time (1/$\gamma$ units)")
